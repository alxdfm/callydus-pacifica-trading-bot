...
| FM-014 | IN_REVIEW | implementacao | P0 | Levar a avaliacao de sinais para loop real com regras finais de produto | Validar localmente o loop real do worker e a gravacao deduplicada de `SignalDecision` antes de seguir para a execucao de ordens em `FM-015`. |
