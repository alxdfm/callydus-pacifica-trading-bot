...
| BG-026 | DROPPED | melhoria | P1 | market data / observabilidade | medir impacto real da centralizacao em `429`, volume de chamadas externas e tempo medio de refresh | descartado: medicao quantitativa nao e prioritaria para o hackathon; retomar se houver sinal de pressao real em producao |
| BG-027 | DROPPED | implementacao | P1 | market data / operacao | automatizar politica de retencao e cleanup operacional fora do uso manual local | descartado: item de operacao continua pos-hackathon; retomar junto ao BG-024 em ambiente de producao persistente |
...
| BG-029 | DONE | decisao em aberto | P0 | runtime / start bot | formalizar o `Start bot readiness check` como gate operacional de produto antes do `resume` | implementado em `StartBotReadinessCheck` e `ResumeBot`; gate obrigatorio validando saldo, sizing, min_order e leverage antes de qualquer `resume` |
...
| BG-031 | DONE | mudanca de escopo | P0 | presets / strategy builder | introduzir `YOUR Strategy` como estrategia custom criada do zero pelo usuario final | feature entregue como unico caminho de estrategia do produto; presets padrao removidos do escopo |
| BG-032 | DONE | implementacao | P0 | presets / strategy builder | implementar o fluxo funcional de `YOUR Strategy`, com registro unico por conta, builder, backtest e ativacao real | implementado com persistencia, materializacao de contrato, preview de backtest, ativacao e runtime real validados ponta a ponta |
| BG-033 | DONE | ajuste de UX | P0 | presets / strategy builder | desenhar a UX guiada de `YOUR Strategy` com `wizard`, warning e preview final | wizard com steps condicionais, resumo legivel, mensagens de correcao e modal dedicado entregues |
| BG-034 | DONE | qualidade | P1 | presets / strategy builder | validar `YOUR Strategy` em usabilidade, regras de negocio e fluxo operacional real | validado em fluxo real com Pacifica: draft, save, backtest, ativacao e runtime sem branch especial |
